...
try:
    ...
    print("=" * 80)
    print("COMPREHENSIVE CODE ANALYSIS - All Bugs Detected")
    print("=" * 80)
    print(f"\n✓ Total Bugs Found: {data['bugs_found']}\n")
    
    if data['bugs_found'] > 0:
        for idx, bug in enumerate(data['bugs'], 1):
            sev_bar = "█" * bug['severity'] + "░" * (5 - bug['severity'])
            print(f"{idx}. [{sev_bar}] {bug['category'].upper()} - Severity {bug['severity']}/5")
            print(f"   {bug['comment']}")
            print(f"   Fix: {bug['fix'][:60]}...")
            print()
    
    print("=" * 80)
    print(f"Most Critical Bug: {data['primary_bug']['category'].upper()} (Sev: {data['primary_bug']['severity']}/5)")
    print("=" * 80)
